 ...
 */
public class PerformanceTest2 extends ServerTests {
		...
		for (String modelName : MODELS) {
			...
			for (int i = 0; i < NUM_ITERATIONS; i++) {
				...
				try {
					...
					memoryMeter.startMeasurements();
					time = System.currentTimeMillis();
					EObject eObject = loadXMLResource(ModelUtil.getResourceLoadOptions());
				...
				try {
					...
					EObject eObject = loadXMLResource(loadOptions);
	...
	private static String saveXMLResource(EObject object, Map<?, ?> options) throws SerializationException {
		...
		// } else {
		res = (XMIResource) (new ResourceSetImpl()).createResource(URI.createFileURI("test.xml"));
		((ResourceImpl) res).setIntrinsicIDToEObjectMap(new HashMap<String, EObject>());
	...
	private static EObject loadXMLResource(Map<?, ?> options) throws SerializationException {
		XMIResource res = (XMIResource) (new ResourceSetImpl()).createResource(URI.createFileURI("test.xml"));
		// ((ResourceImpl) res).setIntrinsicIDToEObjectMap(new HashMap<String, EObject>());
		try {
	...
	private static String saveBinaryResource(EObject object) throws SerializationException {
		...
		// } else {
		res = new BinaryResourceImpl(URI.createFileURI("test.txt"));
		((ResourceImpl) res).setIntrinsicIDToEObjectMap(new HashMap<String, EObject>());
		res.getContents().add(object);
		// res.getContents().add()
		// }
	...
	private static EObject loadBinaryResource(String string) throws SerializationException {
		BinaryResourceImpl res = new BinaryResourceImpl(URI.createFileURI("test.txt"));
		...
		try {
			...
			// res.load(new InputSource(new StringReader(object)), ModelUtil.getResourceLoadOptions());
			res.load(ModelUtil.getResourceLoadOptions());
		} catch (UnsupportedEncodingException e) {
			throw new SerializationException(e);
		} catch (IOException e) {
			throw new SerializationException(e);
